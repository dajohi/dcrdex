...
export default class Application {
  constructor() {
    ...
    window.dumpLogger = (loggerID) => {
      ...
      a.href = `data:application/octet-stream;base64,${window.btoa(
  ...
  async loadPage(page, data, skipPush) {
    ...
    if (!skipPush) {
      const path =
        delivered === requestedHandler ? url.toString() : `/${delivered}`;
    ...
    ancestor.querySelectorAll("[data-tooltip]").forEach((el) => {
      bind(el, "mouseenter", () => {
        ...
        this.tooltip.style.top = `${
          lyt.bodyTop - this.tooltip.offsetHeight - 5
        }px`;
      });
      bind(el, "mouseleave", () => {
        this.tooltip.style.left = "-10000px";
  ...
  attachHeader() {
    ...
    bind(pg.innerNoteIcon, "click", () => {
      Doc.hide(pg.noteBox);
    });
    bind(pg.innerProfileIcon, "click", () => {
      Doc.hide(pg.profileBox);
    });
  ...
  showDropdown(icon, dialog) {
    ...
    const hide = (e) => {
      if (!Doc.mouseInElement(e, dialog)) {
        ...
        if (
          dialog === this.page.noteBox &&
          Doc.isDisplayed(this.page.noteList)
        ) {
          this.ackNotes();
        }
      }
  ...
  setNoteTimes(noteList) {
    for (const el of Array.from(noteList.children)) {
      el.querySelector("span.note-time").textContent = Doc.timeSince(
        el.note.stamp
      );
    }
  }
  ...
  notify(note) {
    ...
    switch (note.type) {
      ...
      case "walletconfig": {
        ...
        const balances = this.main.querySelectorAll(
          `[data-balance-target="${wallet.assetID}"]`
        );
        balances.forEach((el) => {
          el.textContent = (wallet.balance.available / 1e8).toFixed(8);
        });
        break;
      }
      case "match": {
    ...
    if (this.showPopups) {
      ...
      Doc.tmplElement(
        span,
        "text"
      ).textContent = `${note.subject}: ${note.details}`;
  ...
  prependNoteElement(note, skipSave) {
    ...
    if (
      this.notes.length === 0 ||
      (Doc.isDisplayed(this.page.noteBox) && Doc.isDisplayed(noteList))
    )
      return;
    let unacked = 0;
  ...
  makeNote(note) {
    ...
    if (note.severity > ntfn.POKE) {
      const cls =
        note.severity === ntfn.SUCCESS
          ? "good"
          : note.severity === ntfn.WARNING
          ? "warn"
          : "bad";
  ...
  makePoke(note) {
    ...
    Doc.tmplElement(
      el,
      "dateTime"
    ).textContent = `${d.toLocaleDateString()}, ${d.toLocaleTimeString()}`;
    Doc.tmplElement(
      el,
      "details"
    ).textContent = `${note.subject}: ${note.details}`;
  ...
  checkResponse(resp, skipNote) {
    if (!resp.requestSuccessful || !resp.ok) {
      if (this.user.inited && !skipNote)
        this.notify(ntfn.make("API error", resp.msg, ntfn.ERROR));
      return false;
    }
    return true;
  }
