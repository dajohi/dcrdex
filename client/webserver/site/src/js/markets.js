...
export default class MarketsPage extends BasePage {
  constructor(application, main, data) {
    ...
    const reporters = {
      ...
      mouse: (d) => {
        this.reportMousePosition(d);
      },
      zoom: (z) => {
        this.reportZoom(z);
      }
    };
    this.chart = new DepthChart(
      page.marketChart,
      reporters,
      State.fetch(depthZoomKey)
    );
    ...
    // Set up the BalanceWidget.
    {
      ...
      bind(wgt.base.connect, "click", () => {
        this.showOpen(this.market.base, this.walletUnlocked);
      });
      bind(wgt.quote.connect, "click", () => {
        this.showOpen(this.market.quote, this.walletUnlocked);
      });
      bind(wgt.base.expired, "click", () => {
        this.showOpen(this.market.base, this.walletUnlocked);
      });
      bind(wgt.quote.expired, "click", () => {
        this.showOpen(this.market.quote, this.walletUnlocked);
      });
      bind(baseIcons.sleeping, "click", () => {
        this.showOpen(this.market.base, this.walletUnlocked);
      });
      bind(quoteIcons.sleeping, "click", () => {
        this.showOpen(this.market.quote, this.walletUnlocked);
      });
      bind(baseIcons.locked, "click", () => {
        this.showOpen(this.market.base, this.walletUnlocked);
      });
      bind(quoteIcons.locked, "click", () => {
        this.showOpen(this.market.quote, this.walletUnlocked);
      });
      bind(wgt.base.newWalletBttn, "click", () => {
        this.showCreate(this.market.base);
      });
      bind(wgt.quote.newWalletBttn, "click", () => {
        this.showCreate(this.market.quote);
      });
    }
      ...
      this.depthLines.input = [
        {
          ...
          color: this.isSell()
            ? this.chart.theme.sellLine
            : this.chart.theme.buyLine
        }
      ];
      this.drawChartLines();
    });
    ...
    ws.registerRoute(bookRoute, (data) => {
      this.handleBookRoute(data);
    });
    // Handle the new order for the order book on the 'book_order' route.
    ws.registerRoute(bookOrderRoute, (data) => {
      this.handleBookOrderRoute(data);
    });
    // Remove the order sent on the 'unbook_order' route from the orderbook.
    ws.registerRoute(unbookOrderRoute, (data) => {
      this.handleUnbookOrderRoute(data);
    });
    // Update the remaining quantity on a booked order.
    ws.registerRoute(updateRemainingRoute, (data) => {
      this.handleUpdateRemainingRoute(data);
    });
    // Handle the new order for the order book on the 'epoch_order' route.
    ws.registerRoute(epochOrderRoute, (data) => {
      this.handleEpochOrderRoute(data);
    });
    // Bind the wallet unlock form.
    bindOpenWallet(app, page.openForm, async () => {
      this.openFunc();
    });
    // Create a wallet
    this.walletForm = new NewWalletForm(app, page.walletForm, async () => {
      this.createWallet();
    });
    // Main order form
    ...
    // Cancel order form
    bindForm(page.cancelForm, page.cancelSubmit, async () => {
      this.submitCancel();
    });
    ...
    bind(page.lotField, "change", () => {
      this.lotChanged();
    });
    bind(page.lotField, "keyup", () => {
      this.lotChanged();
    });
    bind(page.qtyField, "change", () => {
      this.quantityChanged(true);
    });
    bind(page.qtyField, "keyup", () => {
      this.quantityChanged(false);
    });
    bind(page.mktBuyField, "change", () => {
      this.marketBuyChanged();
    });
    bind(page.mktBuyField, "keyup", () => {
      this.marketBuyChanged();
    });
    bind(page.rateField, "change", () => {
      this.rateFieldChanged();
    });
    bind(page.rateField, "keyup", () => {
      this.previewQuoteAmt(true);
    });

    // Market search input bindings.
    bind(page.marketSearch, "change", () => {
      this.filterMarkets();
    });
    bind(page.marketSearch, "keyup", () => {
      this.filterMarkets();
    });
    ...
    this.notifiers = {
      order: (note) => {
        this.handleOrderNote(note);
      },
      epoch: (note) => {
        this.handleEpochNote(note);
      },
      conn: (note) => {
        this.marketList.setConnectionStatus(note);
      },
      balance: (note) => {
        this.handleBalanceNote(note);
      },
      feepayment: (note) => {
        this.handleFeePayment(note);
      }
    };
    ...
    if (
      !selected ||
      !this.marketList.exists(selected.host, selected.base, selected.quote)
    ) {
  ...
  resolveOrderFormVisibility() {
    ...
    const hasWallets =
      base &&
      app.assets[base.id].wallet &&
      quote &&
      app.assets[quote.id].wallet;
  ...
  setLoaderMsgVisibility() {
    ...
    const symbol =
      (!base && this.market.baseCfg.symbol) ||
      (!quote && this.market.quoteCfg.symbol);
  ...
  updateRegistrationStatusView(
    ...
    if (feePaid) {
      this.setRegistrationStatusView(
        "Registration fee payment successful!",
        "",
        "completed"
      );
      return;
  ...
  setOrderBttnText() {
    if (this.isSell()) {
      ...
      this.page.submitBttn.textContent = `Place order to buy  ${this.market.base.symbol.toUpperCase()}`;
  }
  ...
  preBuy() {
    ...
    if (quoteWallet.balance.available < aLot)
      mkt.maxBuys[rate] = { lots: 0, value: 0 };
    if (mkt.maxBuys[rate]) {
  ...
  setMaxOrder(maxOrder, rate) {
    ...
    page.maxFromTicker.textContent = sell
      ? this.market.base.symbol
      : this.market.quote.symbol.toUpperCase();
    // Could subtract the maxOrder.redemptionFees here.
    page.maxToAmt.textContent = Doc.formatCoinValue(
      (maxOrder.value / 1e8) * rate
    );
    page.maxToTicker.textContent = sell
      ? this.market.quote.symbol
      : this.market.base.symbol.toUpperCase();
  }
  ...
  setMarketBuyOrderEstimate() {
    ...
    if (gap) {
      this.page.minMktBuy.textContent = Doc.formatCoinValue(
        (lotSize * buffer * gap) / 1e8
      );
    }
  }
  ...
  refreshActiveOrders() {
    ...
    const orders = app.orders(
      market.dex.host,
      marketID(market.baseCfg.symbol, market.quoteCfg.symbol)
    );
    ...
    for (const ord of orders) {
      ...
      if (
        ...
        ord.tif === Order.StandingTiF &&
        ord.status < Order.StatusExecuted
      ) {
  ...
  handleBookRoute(note) {
    ...
    page.lotSize.textContent = Doc.formatCoinValue(
      market.baseCfg.lotSize / 1e8
    );
    page.rateStep.textContent = market.quoteCfg.rateStep / 1e8;
    this.baseUnits.forEach((el) => {
      el.textContent = b.symbol.toUpperCase();
    });
    this.quoteUnits.forEach((el) => {
      el.textContent = q.symbol.toUpperCase();
    });
  ...
  showVerify() {
    ...
    if (order.isLimit) {
      ...
      page.vTotal.textContent = Doc.formatCoinValue(
        ((order.rate / 1e8) * order.qty) / 1e8
      );
    } else {
  ...
  showCancel(row, orderID) {
    ...
    const symbol = Order.isMarketBuy(order)
      ? this.market.quote.symbol
      : this.market.base.symbol;
  ...
  handleOrderNote(note) {
    ...
    if (
      (oldStatus === Order.StatusEpoch &&
        order.status === Order.StatusBooked) ||
      (oldStatus === Order.StatusBooked && order.status > Order.StatusBooked)
    )
      this.setMarkers();
  }
  ...
  handleEpochNote(note) {
    ...
    for (const metaOrder of Object.values(this.metaOrders)) {
      ...
      switch (true) {
        case order.type === Order.Limit &&
          order.status === Order.StatusEpoch &&
          alreadyMatched:
          statusTD.textContent =
            order.tif === Order.ImmediateTiF ? "executed" : "booked";
          order.status =
            order.tif === Order.ImmediateTiF
  ...
  loadTableSide(sell) {
    ...
    bookSide.forEach((order) => {
      tbody.appendChild(this.orderTableRow(order, cssClass));
    });
  }
  ...
  filterMarkets() {
    ...
    const filter = filterTxt
      ? (mkt) => mkt.name.includes(filterTxt)
      : () => true;
...
class MarketRow {
  constructor(tmpl, mkt) {
    ...
    row.append(
      `${mkt.basesymbol.toUpperCase()}-${mkt.quotesymbol.toUpperCase()}`
    );
  }
}
...
class BalanceWidget {
  ...
  updateWallet(side) {
    ...
    side.locked.textContent = Doc.formatCoinValue(
      (bal.locked + bal.contractlocked) / 1e8
    );
    ...
    const expired =
      new Date().getTime() - new Date(bal.stamp).getTime() > anHour;
    if (expired) {
...
function updateUserOrderRow(tr, ord) {
  ...
  updateDataCol(
    tr,
    "settled",
    `${((Order.settled(ord) / ord.qty) * 100).toFixed(1)}%`
  );
