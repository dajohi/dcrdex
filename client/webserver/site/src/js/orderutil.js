...
export function sellString(ord) {
  return ord.sell ? "sell" : "buy";
}
export function typeString(ord) {
  return ord.type === Limit
    ? ord.tif === ImmediateTiF
      ? "limit (i)"
      : "limit"
    : "market";
}
export function rateString(ord) {
...
export function statusString(order) {
  ...
  switch (order.status) {
    ...
    case StatusEpoch:
      return "epoch";
    case StatusBooked:
      return order.cancelling ? "cancelling" : "booked";
    case StatusExecuted:
      return isLive ? "settling" : "executed";
    case StatusCanceled:
      return isLive ? "canceled/settling" : "canceled";
    case StatusRevoked:
      return isLive ? "revoked/settling" : "revoked";
  }
}
...
export function settled(order) {
  ...
  return order.matches.reduce((settled, match) => {
    ...
    const redeemed =
      (match.side === Maker && match.status >= MakerRedeemed) ||
