...
export default class OrderPage extends BasePage {
  ...
  processMatch(m) {
    ...
    for (const div of Array.from(
      this.page.matchBox.querySelectorAll(".match-card")
    )) {
    ...
    Doc.tmplElement(card, "status").textContent = Order.matchStatusString(
      m.status,
      m.side
    );
  }
...
function inCounterSwapCast(m) {
  return (
    (m.side === Order.Taker && m.status === Order.MakerSwapCast) ||
    (m.side === Order.Maker && m.status === Order.TakerSwapCast)
  );
}
...
function inSwapCast(m) {
  return (
    (m.side === Order.Maker && m.status === Order.MakerSwapCast) ||
