...
export default class WalletsPage extends BasePage {
  constructor(application, body) {
    ...
    this.walletForm = new NewWalletForm(app, page.walletForm, () => {
    ...
    bindOpenWallet(app, page.openForm, () => {
      this.openWalletSuccess();
    });

    // Bind the withdraw form.
    bindForm(page.withdrawForm, page.submitWithdraw, () => {
      this.withdraw();
    });
    ...
    for (const [k, asset] of Object.entries(rowInfos)) {
      ...
      bind(a.connect, "click", (e) => {
        run(e, this.doConnect.bind(this));
      });
      bind(a.withdraw, "click", (e) => {
        run(e, this.showWithdraw.bind(this));
      });
      bind(a.deposit, "click", (e) => {
        run(e, this.showDeposit.bind(this));
      });
      bind(a.create, "click", (e) => {
        run(e, this.showNewWallet.bind(this));
      });
      bind(a.unlock, "click", (e) => {
        run(e, this.showOpen.bind(this));
      });
      bind(a.lock, "click", async (e) => {
        run(e, this.lock.bind(this));
      });
      bind(a.settings, "click", (e) => {
        run(e, this.showReconfig.bind(this));
      });
    }

    // New deposit address button.
    bind(page.newDepAddrBttn, "click", async () => {
      this.newDepositAddress();
    });
    ...
    // form.
    bind(page.changePW, "click", () => {
      this.showChangePW();
    });

    // Submit the password change form.
    bindForm(page.walletRepw, page.submitRepw, () => {
      this.submitChangePW();
    });
    ...
    this.notifiers = {
      ...
      walletstate: (note) => {
        this.handleWalletStateNote(note);
      },
      walletconfig: (note) => {
        this.handleWalletStateNote(note);
      }
  ...
  async showDeposit(assetID) {
    ...
    if (!wallet) {
      app.notify(
        ntfn.make(
          `No wallet found for ${asset.info.name}`,
          "Cannot retrieve deposit address.",
          ntfn.ERROR
        )
      );
      return;
  ...
  async showWithdraw(assetID) {
    ...
    page.withdrawAvail.textContent = (wallet.balance.available / 1e8).toFixed(
      8
    );
  ...
  handleBalanceNote(note) {
    const td = this.page.walletTable.querySelector(
      `[data-balance-target="${note.assetID}"]`
    );
