...
export class NewWalletForm {
  constructor(application, form, success) {
    ...
    this.subform = new WalletConfigForm(
      application,
      fields.walletSettings,
      true
    );
...
export class WalletConfigForm {
  ...
  async fileInputChanged() {
    ...
    const [loadedOpts, defaultOpts] = [
      this.loadedSettings.children.length,
      this.defaultSettings.children.length
    ];
    if (loadedOpts === 0) Doc.hide(this.loadedSettings, this.loadedSettingsMsg);
    if (defaultOpts === 0)
      Doc.hide(this.defaultSettings, this.defaultSettingsMsg);
    if (loadedOpts + defaultOpts === 0)
      Doc.hide(this.showOther, this.otherSettings);
  }
  ...
  update(walletInfo) {
    ...
    Doc.hide(
      this.loadedSettingsMsg,
      this.loadedSettings,
      this.defaultSettingsMsg,
    ...
    const addOpt = (box, opt) => {
      ...
      const el = opt.isboolean
        ? this.checkboxTmpl.cloneNode(true)
        : this.textInputTmpl.cloneNode(true);
