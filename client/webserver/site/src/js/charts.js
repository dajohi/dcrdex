...
export class DepthChart {
  ...
  resize_() {
    ...
    const plotExtents = new Extents(
      yGuess,
      this.canvas.width,
      10,
      this.canvas.height - xLblHeight
    );
    const xLblExtents = new Extents(
      yGuess,
      this.canvas.width,
      this.canvas.height - xLblHeight,
      this.canvas.height
    );
    const yLblExtents = new Extents(
      0,
      yGuess,
      10,
      this.canvas.height - xLblHeight
  ...
  draw() {
    ...
    for (let i = 0; i < sells.length; i++) {
      ...
      while (
        sellMarkers.length &&
        floatCompare(sellMarkers[0].rate, ord.rate)
      ) {
    ...
    const maxY =
      (epochSellSum && epochBuySum
        ? Math.max(epochBuySum, epochSellSum)
        : epochSellSum || epochBuySum || 1) * growthFactor;
    ...
    const yLabels = makeLabels(
      ctx,
      this.plotRegion.height(),
      dataExtents.y.min,
      dataExtents.y.max,
      50,
      this.lotSize,
      this.book.baseSymbol
    );
    ...
    const xLabels = makeLabels(
      ctx,
      this.plotRegion.width(),
      dataExtents.x.min,
      dataExtents.x.max,
      100,
      this.rateStep,
      `${this.book.quoteSymbol}/${this.book.baseSymbol}`
    );
    ...
    this.plotRegion.plot(dataExtents, (ctx, tools) => {
      ...
      ctx.fillText(
        `${((gapWidth / midGap) * 100).toFixed(2)}% spread`,
        tools.x(midGap),
        tools.y(y) + 24
      );
      ...
      this.zoomOutBttn.plot(new Extents(0, 1, 0, 1), (ctx, tools) => {
        ...
        ctx.fillText(
          minusChar,
          this.zoomOutBttn.extents.midX,
          this.zoomOutBttn.extents.midY
        );
      });
      ...
      this.zoomInBttn.plot(new Extents(0, 1, 0, 1), (ctx, tools) => {
        ...
        ctx.fillText(
          plusChar,
          this.zoomInBttn.extents.midX,
          this.zoomInBttn.extents.midY
        );
      });
      ...
      for (const marker of markers || []) {
        const hovered =
          mousePos &&
          withinTolerance(marker.rate, tools.unx(mousePos.x), tolerance);
        if (hovered) hoverMarkers.push(marker.rate);
        ctx.save();
        ctx.lineWidth = hovered || marker.active ? 5 : 3;
        ctx.strokeStyle = marker.sell
          ? this.theme.sellLine
          : this.theme.buyLine;
    ...
    if (mouseData) {
      this.plotRegion.plot(dataExtents, (ctx, tools) => {
        dot(
          ctx,
          tools.x(mouseData.rate),
          tools.y(mouseData.depth),
          mouseData.dotColor,
          5
        );
      });
