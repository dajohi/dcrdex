...
// asset.Coin interface.
func (output *output) Confirmations(ctx context.Context) (uint32, error) {
	txOut, err := output.node.GetTxOut(ctx, &output.txHash, output.vout, true)
	if err != nil {
...
func (d *Driver) Setup(ctx context.Context, cfg *asset.WalletConfig, logger dex.Logger, network dex.Network) (asset.Wallet, error) {
	return NewWallet(ctx, cfg, logger, network)
}
...
// canceled.
func NewWallet(_ context.Context, cfg *asset.WalletConfig, logger dex.Logger, network dex.Network) (*ExchangeWallet, error) {
...
func (dcr *ExchangeWallet) Connect(ctx context.Context) (error, *sync.WaitGroup) {
	...
	if atomic.SwapUint32(&dcr.hasConnected, 1) == 0 {
		err := dcr.walletNode.LockUnspent(ctx, true, nil)
		if err != nil {
			return err, nil
		}
	}
...
func (dcr *ExchangeWallet) Fund(ctx context.Context, value uint64, nfo *dex.Asset) (asset.Coins, error) {
	if value == 0 {
	...
	coins, _, _, err := dcr.fund(ctx, nfo.FundConf, enough)
	return coins, err
}
...
// needed value.
func (dcr *ExchangeWallet) fund(ctx context.Context, confs uint32,
...
out:
	...
	err = dcr.lockFundingCoins(ctx, spents)
	if err != nil {
		return nil, 0, 0, err
	}
...
func (dcr *ExchangeWallet) lockFundingCoins(ctx context.Context, fCoins []*fundingCoin) error {
	...
	err := dcr.walletNode.LockUnspent(ctx, false, wireOPs)
	if err != nil {
		return err
	}
	dcr.fundingMtx.Lock()
...
// canceled order.
func (dcr *ExchangeWallet) ReturnCoins(ctx context.Context, unspents asset.Coins) error {
	if len(unspents) == 0 {
	...
	for _, unspent := range unspents {
		op, err := dcr.convertCoin(ctx, unspent)
		if err != nil {
	...
	return dcr.walletNode.LockUnspent(ctx, true, ops)
}

// FundingCoins gets funding coins for the coin IDs. The coins are locked. This
// method might be called to reinitialize an order from data stored externally.
// This method will only return funding coins, e.g. unspent transaction outputs.
func (dcr *ExchangeWallet) FundingCoins(ctx context.Context, ids []dex.Bytes) (asset.Coins, error) {
	...
	err = dcr.walletNode.LockUnspent(ctx, false, lockers)
	if err != nil {
		return nil, err
	}
	return coins, nil
...
func (dcr *ExchangeWallet) Swap(ctx context.Context, swaps *asset.Swaps, nfo *dex.Asset) ([]asset.Receipt, asset.Coin, error) {
	var totalOut uint64
	// Start with an empty MsgTx.
	baseTx := wire.NewMsgTx()
	// Add the funding utxos.
	totalIn, err := dcr.addInputCoins(ctx, baseTx, swaps.Inputs)
	if err != nil {
		return nil, nil, err
	}
	...
	for _, contract := range swaps.Contracts {
		...
		// abandoned.
		revokeAddr, err := dcr.walletNode.GetNewAddressGapPolicy(ctx, dcr.acct, dcrwallet.GapPolicyIgnore)
		if err != nil {
	...
	changeAddr, err := dcr.walletNode.GetRawChangeAddress(ctx, dcr.acct, chainParams)
	if err != nil {
		return nil, nil, fmt.Errorf("error creating change address: %v", err)
	}
	// Add change, sign, and send the transaction.
	msgTx, change, err := dcr.sendWithReturn(ctx, baseTx, changeAddr, totalIn, totalOut, nfo.FeeRate, nil)
	if err != nil {
		return nil, nil, err
	}
...
// redemption.
func (dcr *ExchangeWallet) Redeem(ctx context.Context, redemptions []*asset.Redemption, nfo *dex.Asset) ([]dex.Bytes, asset.Coin, error) {
	...
	redeemAddr, err := dcr.walletNode.GetRawChangeAddress(ctx, dcr.acct, chainParams)
	if err != nil {
	...
	txHash, err := dcr.node.SendRawTransaction(ctx, msgTx, false)
	if err != nil {
		return nil, nil, err
	}
	if *txHash != checkHash {
...
func (dcr *ExchangeWallet) SignMessage(ctx context.Context, coin asset.Coin, msg dex.Bytes) (pubkeys, sigs []dex.Bytes, err error) {
	output, err := dcr.convertCoin(ctx, coin)
	if err != nil {
	...
	if found {
		...
		txOut, err := dcr.node.GetTxOut(ctx, &output.txHash, output.vout, true)
	...
	priv, pub, err := dcr.getKeys(ctx, address)
	if err != nil {
		return nil, nil, err
	}
...
// during a swap.
func (dcr *ExchangeWallet) AuditContract(ctx context.Context, coinID, contract dex.Bytes) (asset.AuditInfo, error) {
...
func (dcr *ExchangeWallet) FindRedemption(ctx context.Context, coinID dex.Bytes) (dex.Bytes, error) {
	...
	tx, err := dcr.walletNode.GetTransaction(ctx, txHash)
	if err != nil {
	...
	if tx.Confirmations == 0 {
		...
		rawTx, err := dcr.node.GetRawTransactionVerbose(ctx, txHash)
		if err != nil {
		...
		txs, err := dcr.node.GetRawMempool(ctx, chainjson.GRMAll)
		if err != nil {
	...
	for {
		...
		block, err := dcr.node.GetBlockVerbose(ctx, blockHash, true)
		if err != nil {
...
// expired.
func (dcr *ExchangeWallet) Refund(ctx context.Context, receipt asset.Receipt, nfo *dex.Asset) error {
	op := receipt.Coin()
	...
	utxo, err := dcr.node.GetTxOut(ctx, txHash, vout, true)
	if err != nil {
	...
	refundAddr, err := dcr.walletNode.GetNewAddressGapPolicy(ctx, dcr.acct, dcrwallet.GapPolicyIgnore)
	if err != nil {
	...
	msgTx.AddTxOut(txOut)
	// Sign it.
	refundSig, refundPubKey, err := dcr.createSig(ctx, msgTx, 0, redeem, sender)
	if err != nil {
		return err
	}
	...
	// Send it.
	checkHash := msgTx.TxHash()
	refundHash, err := dcr.node.SendRawTransaction(ctx, msgTx, false)
	if err != nil {
		return err
	}
...
func (dcr *ExchangeWallet) Address(ctx context.Context) (string, error) {
	addr, err := dcr.walletNode.GetNewAddressGapPolicy(ctx, dcr.acct, dcrwallet.GapPolicyIgnore)
	if err != nil {
		return "", err
	}
...
func (dcr *ExchangeWallet) Unlock(ctx context.Context, pw string, dur time.Duration) error {
	return dcr.walletNode.WalletPassphrase(ctx, pw, int64(dur/time.Second))
}

// Lock locks the exchange wallet.
func (dcr *ExchangeWallet) Lock(ctx context.Context) error {
	return dcr.walletNode.WalletLock(ctx)
}

// PayFee sends the dex registration fee. Transaction fees are in addition to
// the registration fee, and the fee rate is taken from the DEX configuration.
func (dcr *ExchangeWallet) PayFee(ctx context.Context, address string, regFee uint64, nfo *dex.Asset) (asset.Coin, error) {
	addr, err := dcrutil.DecodeAddress(address, chainParams)
	if err != nil {
		return nil, err
	}
	msgTx, sent, err := dcr.sendRegFee(ctx, addr, regFee, nfo)
	if err != nil {
		return nil, err
	}
	if sent != regFee {
...
// the value.
func (dcr *ExchangeWallet) Withdraw(ctx context.Context, address string, value, feeRate uint64) (asset.Coin, error) {
	addr, err := dcrutil.DecodeAddress(address, chainParams)
	if err != nil {
		return nil, err
	}
	msgTx, net, err := dcr.sendMinusFees(ctx, addr, value, feeRate)
	if err != nil {
		return nil, err
	}
...
func (dcr *ExchangeWallet) addInputCoins(ctx context.Context, msgTx *wire.MsgTx, coins asset.Coins) (uint64, error) {
	var totalIn uint64
...
func (dcr *ExchangeWallet) shutdown(ctx context.Context) {
...
func (dcr *ExchangeWallet) convertCoin(ctx context.Context, coin asset.Coin) (*output, error) {
	op, _ := coin.(*output)
	if op != nil {
		return op, nil
	}
...
// sent value.
func (dcr *ExchangeWallet) sendMinusFees(ctx context.Context, addr dcrutil.Address, val, feeRate uint64) (*wire.MsgTx, uint64, error) {
	if val == 0 {
	...
	coins, _, _, err := dcr.fund(ctx, 0, enough)
	if err != nil {
		return nil, 0, err
	}
	tx, sent, err := dcr.sendCoins(ctx, addr, coins, val, feeRate, true)
	if err != nil {
		return nil, 0, err
	}
	return tx, sent, nil
...
// output.
func (dcr *ExchangeWallet) sendRegFee(ctx context.Context, addr dcrutil.Address, regfee uint64, nfo *dex.Asset) (*wire.MsgTx, uint64, error) {
	coins, err := dcr.Fund(ctx, regfee, nfo)
	if err != nil {
		return nil, 0, err
	}
	return dcr.sendCoins(ctx, addr, coins, regfee, nfo.FeeRate, false)
}
...
func (dcr *ExchangeWallet) sendCoins(ctx context.Context, addr dcrutil.Address, coins asset.Coins, val, feeRate uint64, subtract bool) (*wire.MsgTx, uint64, error) {
	baseTx := wire.NewMsgTx()
	totalIn, err := dcr.addInputCoins(ctx, baseTx, coins)
	if err != nil {
		return nil, 0, err
	}
	...
	tx, _, err := dcr.sendWithReturn(ctx, baseTx, changeAddr, totalIn, val, feeRate, subtractee)
...
// change output.
func (dcr *ExchangeWallet) sendWithReturn(ctx context.Context, baseTx *wire.MsgTx,
	...
	msgTx, signed, err := dcr.walletNode.SignRawTransaction(ctx, baseTx)
	if err != nil {
...
func (dcr *ExchangeWallet) createSig(ctx context.Context, tx *wire.MsgTx, idx int, pkScript []byte, addr dcrutil.Address) (sig, pubkey []byte, err error) {
	sigTyper, ok := addr.(signatureTyper)
	if !ok {
		return nil, nil, fmt.Errorf("invalid address type")
	}

	priv, pub, err := dcr.getKeys(ctx, addr)
	if err != nil {
		return nil, nil, err
	}
...
func (dcr *ExchangeWallet) getKeys(ctx context.Context, addr dcrutil.Address) (*secp256k1.PrivateKey, *secp256k1.PublicKey, error) {
	wif, err := dcr.walletNode.DumpPrivKey(ctx, addr)
	if err != nil {
		return nil, nil, err
	}
...
func (dcr *ExchangeWallet) monitorBlocks(ctx context.Context) {
	h, _, err := dcr.node.GetBestBlock(ctx)
	if err != nil {
